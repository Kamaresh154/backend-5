...
async def list_parents(
    ...
    items, total = await parent_service.list_parents(
        db, _org_id(current), search=search, page=page, page_size=page_size
    )
...
async def create_parent(
    ...
    parent = await parent_service.create_parent(db, _org_id(current), data)
...
async def get_parent(
    ...
    current.require_permission("parents.read")
    org = _org_id(current)
    parent = await parent_service.get_parent(db, org, parent_id)
    students = await parent_service.get_parent_students(db, org, parent_id)
...
async def update_parent(
    ...
    current.require_permission("parents.write")
    parent = await parent_service.update_parent(db, _org_id(current), parent_id, data)
    return ParentResponse.model_validate(parent)
...
async def delete_parent(parent_id: UUID, current: CurrentUserDep, db: DbSession) -> None:
    current.require_permission("parents.write")
    await parent_service.soft_delete_parent(db, _org_id(current), parent_id)
...
async def link_student(
    ...
    current.require_permission("parents.write")
    await parent_service.link_student(db, _org_id(current), parent_id, data)
...
async def unlink_student(
    ...
    current.require_permission("parents.write")
    await parent_service.unlink_student(db, _org_id(current), parent_id, student_id)
